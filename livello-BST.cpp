...
public:
	...
	int depth(Node<K> *node, K key)
	{
		...
		int r;
		if(key < node->getKey())
			r = depth(node->getLeft(), key);
		else
			r = depth(node->getRight(), key);
	...
	void insert(K key)
	{
		...
		if(key < parent->getKey())			//ATTENZIONE A QUESTO PASSAGGIO
	...
	int livello(Node<K> *node, K key, int level)
	{
		if(node == NULL)
			return -1;
		if(node->getKey() == key)
		...
		int downlevel = livello(node->getLeft(), key, level+1);
		if(downlevel != -1)
			return downlevel;
		
		downlevel = livello(node->getRight(), key, level+1);
...
int main()
{
	...
	for(int i=0; i<100; i++)			//ciclos sulle righe
	{
		in >> data_type >> N >> M;
		...
		if(data_type == "int")
		{
			...
			for(int k=0; k<M; k++)				//ciclo sui numeri di cui stampare il livello
			{
				in >> auxi;
				out << myBST.livello(myBST.getRoot(), auxi, 0) << " ";
			}
		...
		if(data_type == "double")
		{
			...
			{
				in >> auxd;
				out << myBST.livello(myBST.getRoot(), auxd, 0) << " ";
			}
