...
int main()
{
	...
	for(int i=0; i<100; i++)
	{
		...
		if(data_type == "int")
		{
			...
			for(int k=0; k<M; k++)		//leggiamo da file, lo mettiamo nell'array e ne stampiamo il predecessore
			{
				in >> appoggio;
				isucc[k] = atoi(appoggio.c_str());
				
				out << myBST.getPredecessor(isucc[k]) << " ";
			}
		...
		if(data_type == "double")
		{
			...
			for(int k=0; k<M; k++)		//leggiamo da file, lo mettiamo nell'array e ne stampiamo il predecessore
			{
				...
				out << myBST.getPredecessor(dsucc[k]) << " ";
			}
